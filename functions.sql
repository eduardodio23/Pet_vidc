...
-- 2) Total gasto por tutor (soma valores de consultas dos animais desse tutor, exceto canceladas)
...
BEGIN
    ...
    WHERE a.tutor_id = p_tutor_id
      AND (c.status IS NULL OR c.status != 'cancelada');
    RETURN v_total;
END $$
DELIMITER ;
...
DELIMITER $$
CREATE FUNCTION fn_qtd_consultas_animal(p_animal_id INT)mysql -u root -p
USE pet_vida;
...
-- ====== Testes (SELECTs reais) ======
-- 1) Idade dos animais
SELECT nome, fn_idade_animal(data_nascimento) AS idade FROM animais LIMIT 10;

-- 2) Total gasto por tutor (usar coluna id_tutor)
SELECT nome, fn_total_gasto_tutor(id_tutor) AS total_gasto FROM tutores LIMIT 10;

-- 3) Qtde consultas por animal
SELECT nome, fn_qtd_consultas_animal(id_animal) AS qtd_consultas FROM animais LIMIT 10;

-- 4) Status com emoji
SELECT id_consulta, status, fn_status_emoji(status) AS status_emoji FROM consultas LIMIT 10;

-- 5) Classificação de valores
SELECT id_consulta, valor, fn_classificar_valor(valor) AS classificacao FROM consultas LIMIT 10;
